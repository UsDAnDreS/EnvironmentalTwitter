...
account.labels <- read.csv("FINALIZED_Training_Data_ALL_Available_Descriptions_EMOJIS_UNCHANGED.csv")


account.labels <- rbind(account.labels %>%
                          select(username, description, hand.label_simplified),
                        read.csv("SVM_BERT_unweighted_UNLABELED_PREDICTED_accounts_W_PROBABILITIES_emojis_unchanged.csv") %>%
                          select(username, description, hand.label_simplified))
...
####
## Getting sample tweets
####
View(our.df %>% filter(hand.label_simplified == "acad"))
...
#         other  media    gov   acad tourbiz
# other   0.000 -0.110 -0.536 -0.290  -0.185
# media   0.119  0.000 -0.502 -0.214  -0.090
# gov     0.576  0.500  0.000  0.252   0.393
# acad    0.293  0.201 -0.238  0.000   0.116
# tourbiz 0.193  0.087 -0.382 -0.120   0.000
