...
##    The RESULTS OF COMBINING CONVERSATIONS with the ORIGINAL TWEET DATA FRAME
...
load(file="recent.search.dfs.all.cleaned.tweets.RData")
...
for (l in 1:length(main.queries)){
  ...
  for (j in 1:length(area.terms)){
    ...
    conv.vec <- both.tweets.and.retweets[[l]][[j]]$conversation_id[is.na(both.tweets.and.retweets[[l]][[j]]$in_reply_to_user_id)
                                                                   & (both.tweets.and.retweets[[l]][[j]]$public_metrics.x_reply_count > 0)]
    
    if (length(conv.vec) == 0){
      all_queries[[l]][[j]] <- "None"
      next;
    } else {
      conversation_ids[[l]][[j]] <- conv.vec
    }
...
##    2. Add the NON-DUPLICATES to the ORIGINAL "both.tweets.and.retweets[[l]][[j]]" set
#####
...
for (l in 1:length(main.queries)){
  ...
  for (j in 1:length(area.terms)){
    ...
    for (query in all_queries[[l]][[j]]){
      ...
      inter.query <- query_twitter(params,0, type="recent")
...
save(full.df, file="recent.search.dfs.full.UNsorted.RData")
...
save(full.df, file="recent.search.dfs.full.sorted.RData")
