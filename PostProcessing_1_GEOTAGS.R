#######
## For GEOTAGGED TWEETS (no explicit mention of area in the tweet necessary)
#######
...
for (l in 1:length(topic.dfs.geotag)){
  print(paste0("Topic #: ",l, ", ", names(topic.dfs.geotag)[l]))
  ...
  for (i in 1:length(area.terms)){
    ...
    # topic.dfs.geotag[[l]][[i]] <- topic.dfs.geotag[[l]][[i]][-grep("RT ", topic.dfs.geotag[[l]][[i]]$text), ]
    
    
    ## There could be a "retweet-only" data chunk, so...
    
    if (nrow(topic.dfs.geotag[[l]][[i]]) == 0){
      # Creating an empty data set, but with extended column names
      cnames <- colnames(topic.dfs.geotag[[l]][[i]])
      topic.dfs.geotag[[l]][[i]] <- data.frame(matrix(ncol=ncol(topic.dfs.geotag[[l]][[i]]) + 1, nrow=0))
      colnames(topic.dfs.geotag[[l]][[i]]) <- c(cnames, "text_with_display_links")
      next;
    }
    ...
    #topic.dfs.geotag[[l]][[i]]$entities.x_urls
    
    
    pattern <- 'https://t.co/[a-zA-Z0-9]*'
    m <- regexpr(pattern, topic.dfs.geotag[[l]][[i]]$text)
    # grep(pattern, topic.dfs.geotag[[l]][[i]]$text)
    # regmatches(topic.dfs.geotag[[l]][[i]]$text, m)
    
    # Where "m != -1", the tweet has a link.
    # sum(sapply(topic.dfs.geotag[[l]][[i]][m != -1,]$entities.x_urls, is.null))
    # sum(sapply(topic.dfs.geotag[[l]][[i]][m == -1,]$entities.x_urls, function(x) !is.null(x)))
    ...
    display.and.expanded.urls.list <- lapply(topic.dfs.geotag[[l]][[i]][m != -1,]$entities.x_urls, function(x) x[, c("display_url", "expanded_url")])
    ...
    if  (length(display.and.expanded.urls.list) == 0){
      topic.dfs.geotag[[l]][[i]]$text_with_display_links <- topic.dfs.geotag[[l]][[i]]$text
      next;
    }
    ...
    topic.dfs.geotag[[l]][[i]]$text_with_display_links <- topic.dfs.geotag[[l]][[i]]$text
    topic.dfs.geotag[[l]][[i]]$text <- gsub(pattern, "", topic.dfs.geotag[[l]][[i]]$text)
    ...
    # l <- 1; i <- 1
    # which(sapply(topic.dfs.geotag[[l]][[i]]$entities.x_urls, function(x) any(is.na(x$expanded_url))))
    # topic.dfs.geotag[[l]][[i]]$entities.x_urls[which(sapply(topic.dfs.geotag[[l]][[i]]$entities.x_urls, function(x) any(is.na(x$expanded_url))))]
    ...
    # View(data.frame(topic.dfs.geotag[[l]][[i]]$text))
    # View(data.frame(topic.dfs.geotag[[l]][[i]]$text[m == -1]))
    # topic.dfs.geotag[[l]][[i]]$text[m == -1]
    # topic.dfs.geotag[[l]][[i]][928,]
    ...
    # display.and.expanded.urls.list.extra <- lapply(topic.dfs.geotag[[l]][[i]][m != -1,]$entities.x_urls, function(x){
...
save(topic.dfs.geotag, file="topic.dfs.geotag.expanded.urls.RData")
